...
# Accept credentials as parameters or use defaults
SUBDOMAIN=${1:-"test-admin-2368"}
ADMIN_EMAIL=${2:-""}
ADMIN_PASSWORD=${3:-""}
...
# Step 2: Try to create school or use existing
echo -e "${BLUE}Step 2: School Setup${NC}"
if [ -z "$ADMIN_EMAIL" ] || [ -z "$ADMIN_PASSWORD" ]; then
  echo -e "${YELLOW}⚠️  No admin credentials provided${NC}"
  echo "Attempting to create a new school..."
  
  TIMESTAMP=$(date +%s | cut -c7-12)
  NEW_SUBDOMAIN="test-admin-${TIMESTAMP}"
  SCHOOL_DATA=$(cat <<JSON
{
  "name": "Test Admin School ${TIMESTAMP}",
  "subdomain": "${NEW_SUBDOMAIN}",
  "domain": "${NEW_SUBDOMAIN}.compasse.net",
  "school": {
    "name": "Test Admin School ${TIMESTAMP}",
    "address": "123 Test Street",
    "phone": "+1234567890",
    "email": "info@${NEW_SUBDOMAIN}.compasse.net",
    "website": "https://${NEW_SUBDOMAIN}.compasse.net",
    "admin_name": "Test Admin",
    "admin_email": "admin@${NEW_SUBDOMAIN}.compasse.net"
  },
  "settings": {
...
JSON
  ...
  if [ -n "$ADMIN_EMAIL" ] && [ "$ADMIN_EMAIL" != "null" ]; then
    ...
    echo "Response:"
    echo "$SCHOOL_RESPONSE" | jq '.' | head -10
    echo ""
    echo -e "${YELLOW}⚠️  Cannot proceed without admin credentials${NC}"
    echo "Usage: $0 <subdomain> <admin_email> <admin_password>"
    exit 1
  fi
else
